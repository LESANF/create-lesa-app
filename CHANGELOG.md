...
- `TEMPLATE_REF` → `v0.0.11`. 0.0.10 템플릿은 부팅 때 Uniwind 테마 변수 에러를 냈다(`@variant light` 블록 누락)
- `bin` 경로에서 `./` 를 뺐다 — npm 이 발행 때마다 고쳐 올리며 경고를 냈다

## [0.0.10] — 2026-09-22

### Changed

- `TEMPLATE_REF` → `v0.0.10`. 원시색·의미 토큰 분리와 `useColors()`, Input 구조, Button 동작, Icon 바탕,
